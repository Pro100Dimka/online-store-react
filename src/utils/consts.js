export const GITHUB_SITE = '/online-store-react-js';
export const SHOP_ROUTE = `${GITHUB_SITE}/`;
export const TYPE_ROUTE = '/type';
export const BRAND_ROUTE = '/brand';
export const ADMIN_ROUTE = `${GITHUB_SITE}/admin`;
export const LOGIN_ROUTE = `${GITHUB_SITE}/login`;
export const DEVICE_ROUTE = '/device';
export const BASKET_ROUTE = '/basket';
export const REGISTRATION_ROUTE = `${GITHUB_SITE}/registration`;
export const DEVICES_LIST = '/devices-list';
export const BRANDS_LIST = '/brands-list';
export const TYPES_LIST = '/types-list';
export const SELECTED_TYPE = 'selected-type';
export const SELECTED_BRAND = 'selected-brand';

const PREFIX = 'm-table-toolbar_2';
export const classesTable = {
  root: `${PREFIX}-root`,
  highlight: `${PREFIX}-highlight`,
  spacer: `${PREFIX}-spacer`,
  actions: `${PREFIX}-actions`,
  title: `${PREFIX}-title`,
  searchField: `${PREFIX}-searchField`,
  formControlLabel: `${PREFIX}-formControlLabel`
};
